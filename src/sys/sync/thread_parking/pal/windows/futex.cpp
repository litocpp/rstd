...
bool futex_wait(const std::atomic<T>* futex, T expected, std::chrono::milliseconds timeout) {
    DWORD timeout_ms = timeout == std::chrono::milliseconds::max() ? INFINITE : 
...
template bool futex_wait<uint32_t>(const std::atomic<uint32_t>*, uint32_t, std::chrono::milliseconds);
