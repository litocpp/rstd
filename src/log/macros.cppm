...
inline void log_internal(Level level, ref<str> target, fmt::Arguments args, panic_::Location loc) noexcept {
    if (level <= max_level()) {
        Record rec {
...
struct error {
    error(fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        ...
        detail::log_internal(Level::Error, ref<str>(),
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
    error(Target tgt, fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Error, tgt.value,
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
};
...
struct warn {
    warn(fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Warn, ref<str>(),
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
    warn(Target tgt, fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Warn, tgt.value,
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
};
...
struct info {
    info(fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Info, ref<str>(),
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
    info(Target tgt, fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Info, tgt.value,
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
};
...
struct debug {
    debug(fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Debug, ref<str>(),
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
    debug(Target tgt, fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Debug, tgt.value,
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
};
...
struct trace {
    trace(fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Trace, ref<str>(),
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
    trace(Target tgt, fmt::format_string<Args...> fmt_str, Args&&... args, panic_::SrcLoc loc = {}) {
        fmt::Argument arg_array[] = { fmt::Argument::make(args)... };
        detail::log_internal(Level::Trace, tgt.value,
            { fmt_str.data(), fmt_str.size(), arg_array, sizeof...(Args) },
            panic_::Location::from(loc.val));
    }
};
