...
auto Timespec::checked_seconds(rstd::int128_t value) noexcept -> Option<i64> {
...
auto Timespec::sub_timespec(Timespec other) const noexcept -> rstd::time::Duration {
    if (*this < other) return rstd::time::Duration::from_secs(u64 {});

    auto const difference =
        static_cast<rstd::uint128_t>(static_cast<rstd::int128_t>(tv_sec.to_primitive()) -
...
auto Timespec::checked_add_duration(rstd::time::Duration dur) const noexcept -> Option<Timespec> {
...
auto Timespec::checked_sub_duration(rstd::time::Duration dur) const noexcept -> Option<Timespec> {
    auto seconds = checked_seconds(static_cast<rstd::int128_t>(tv_sec.to_primitive()) -
                                   static_cast<rstd::int128_t>(dur.as_secs().to_primitive()));
    if (seconds.is_none()) return None();
    i64 secs = rstd::move(seconds).unwrap_unchecked();
...
auto operator>=(Timespec a, Timespec b) noexcept -> bool {
...
auto operator<=>(Timespec a, Timespec b) noexcept -> decltype(i64 {} <=> i64 {}) {
...
auto operator<=>(Instant a, Instant b) noexcept -> decltype(i64 {} <=> i64 {}) {
    return a.t <=> b.t;
}
...
auto SystemTime::sub_time(SystemTime other) const noexcept
    ...
    if (t >= other.t) return Ok(t.sub_timespec(other.t));
...
auto operator<=>(SystemTime a, SystemTime b) noexcept -> decltype(i64 {} <=> i64 {}) {
    return a.t <=> b.t;
}
