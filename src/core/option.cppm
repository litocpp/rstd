...
constexpr void reinit(T* newval, U* oldval,
                      U&& arg) noexcept(std::is_nothrow_constructible_v<T, U>) {
...
public:
    constexpr auto is_some() const noexcept -> bool { return cast_self().m_has_val; }

    constexpr auto is_none() const noexcept -> bool { return ! cast_self().m_has_val; }
        ...
        if (o.is_some()) {
            if constexpr (meta::is_reference_v<typename Self::value_type>) {
                return Option(*Impl<clone::Clone,
                                    typename option::detail::option_traits<Self>::union_value_t>::
                                  clone(&o.m_val));
            } else {
                return Option(Impl<clone::Clone,
                                   typename option::detail::option_traits<Self>::union_value_t>::
