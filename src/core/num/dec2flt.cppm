...
constexpr usize DECIMAL_MAX_DIGITS = 800;
...
auto compare_ratio_to_pow2(DecimalBig const& numerator, DecimalBig const& denominator, i32 exponent)
    -> i8 {
    if (exponent >= 0) {
...
auto rounded_quotient(DecimalBig numerator, DecimalBig denominator, bool sticky) -> u64 {
...
auto make_float(DecimalBig numerator, DecimalBig denominator, bool negative, bool sticky)
    ...
    return rstd::Ok(rstd::bit_cast<f64>(bits));
}
...
auto to_f64(Decimal decimal) -> Result<f64, Error> {
    ...
    for (usize i = 0; i < total; ++i) {
        const u8 digit = digit_at(decimal, i);
