...
TEST(Fd, OwnedFdMoveAssignClosesPrevious) {
    int     r1 = fd_open(libc::O_RDONLY);
    int     r2 = fd_open(libc::O_RDONLY);
    OwnedFd a { r1 };
    OwnedFd b { r2 };
    b = rstd::move(a);
...
TEST(Fd, IntoRawFdLeaks) {
    ...
    libc::close(raw);             // manual cleanup
}

TEST(Fd, TryCloneProducesDistinctFd) {
    OwnedFd a { fd_open(libc::O_RDONLY) };
...
TEST(Fd, BorrowedFdRoundTrip) {
    OwnedFd fd { fd_open(libc::O_RDONLY) };
    auto    b = fd.as_fd();
